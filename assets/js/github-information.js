...
function repoInformationHTML(repos) {
  ...
  var listItemsHTML = repos.map(function createRepoList(repo) {
    return `<li><a href="${repo.html_usr}" target="_blank">${repo.name}</a></li>`;
  })
  ...
  $.when(
    ...
    function getData(userResponse, repoResponse) {
      ...
      if (errorResponse.status === 404) {
        ...
        $('#gh-user-data').html(`<h4>To many request, please wait until ${resetTime.toLocaleTimeString()}</h4>`)
      } else {
